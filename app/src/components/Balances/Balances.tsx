...
const Balances = ({ className }: { className?: string }) => {
  ...
  return (
    <div className={`absolute top-full left-0 hidden ${className}`}>
      {balance.map((b) => (
        <div key={b.name} className="inline-flex w-full justify-between items-center bg-neutral-100 border border-neutral-300 focus:outline-none rounded-lg text-base text-black mt-4 md:mt-0">
