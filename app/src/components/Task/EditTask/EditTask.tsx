...
import EditProgressModal from '../ProgressModal/EditProgressModal'
import { IEditTask } from './types'
...
const EditTask: React.FC<IEditTask> = ({
  ...
  const [taskData, setTaskData] = useState({
    ...
    description: task.externalId ? '' : task.description,
    taskTags: task.taskTags.map((tag) => Number(tag.id)),
    complexityScore: task.complexityScore,
    reputationLevel: task.reputationLevel,
    dueDate: moment.unix(task.dueDate).format('YYYY-MM-DD'),
        ...
        <div className='paper w-full md:w-1/2 h-[95vh] px-2 py-5 absolute right-0 top-0 rounded-xl my-5 overflow-hidden'>
          <div className='px-6 w-full'>
            <section className='flex justify-between border border-t-0 border-r-0 border-l-0 pb-3'>
              <span className='block font-bold text-2xl'>{t('edit_task')}</span>
              ...
              <section className='pt-4 pb-5'>
                ...
                <div className='mt-4'>
                  ...
                  <input
                    ...
                    id='title'
                    name='title'
                    value={task.externalId ? task.title : taskData.title}
                    onChange={handleChange}
                    className='w-full border p-2 rounded-md focus:outline-none'
                    required
                    disabled={!!task.externalId}
                  />
                </div>
                <div className='w-full p-3 bg-gray-100 border mt-3 rounded-xl'>
                  ...
                  <textarea
                    ...
                    name='description'
                    value={
                      task.externalId ? task.description : taskData.description
                    }
                    onChange={handleChange}
                    rows={5}
                      ...
                      return (
                        <span key={value}>
                          <input
                            ...
                            disabled={task.status !== TaskStatus.PROPOSED}
                          />
                          <label
                            htmlFor={value}
                            className='cursor-pointer w-[max-content] border text-sm text-center px-4 py-1 rounded-lg focus:bg-blue-700 peer-checked:bg-blue-700 peer-checked:text-white peer-checked:font-medium peer-checked:font-semibold border-b-[1px] border-gray peer-checked:border-blue-500'
